...
test('HTTPS request fails with self-signed cert when no extra trust is provided', async () => {
  const server = createServer({ key: SERVER_KEY, cert: SERVER_CERT }, (req, res) => {
    res.writeHead(200);
    res.end('ok');
  });
  ...
  await expect(
    ...
    const code =
      e.code || (e.cause && e.cause.code) || (typeof e.message === 'string' && e.message);
...
test('HTTPS request succeeds when NODE_EXTRA_CA_CERTS contains the CA', async () => {
  ...
  const script = `
    const { get } = require(${JSON.stringify(REQUEST_MODULE_PATH)});
    (async () => {
      const res = await get({
        url: ${JSON.stringify(url)},
        appName: ${JSON.stringify(requestOptions.appName)},
        instanceId: ${JSON.stringify(requestOptions.instanceId)},
        connectionId: ${JSON.stringify(requestOptions.connectionId)},
        interval: 0,
      });
